...
def transcribe_batch(audio_files, model_size="large-v3"):
    """
    ...
    if input_folder:
        ...
        try:
            ...
            # Remove "audio" prefix if present (e.g., "audio_input/Baka_Survey/Central" → "Baka_Survey/Central")
            if rel_path.startswith("audio" + os.sep):
                rel_path = rel_path[len("audio" + os.sep):]
    ...
    # Load model ONCE for all files (huge performance boost)
    print(f"Loading Whisper {model_size} model (faster-whisper)...")
    try:
        # Try GPU first (CUDA), fall back to CPU if not available
        try:
...
if __name__ == "__main__":
    ...
    # trimmed = trim_audio(audio_file, duration_sec=180)
    # transcribe_thai(trimmed)
    
    # MODE 2: Batch processing - List of files
    audio_files = [
        "audio_input/drItthisek.m4a",
        # "audio_input/another_file.mp3",
        # "audio_input/third_file.wav",
    ]
    # transcribe_batch(audio_files)
    
    # MODE 3: Batch processing - Entire folder
    transcribe_batch("audio_input/Baka_Survey/Central")
    transcribe_batch("audio_input/Baka_Survey/Esan/EarthyOhmy")
    transcribe_batch("audio_input/Baka_Survey/Esan/Title")
